...
h2. TestFlight for Android (Private Beta) support - umi-uyura added.

Only TestFlight upload, not deploy for device.

<pre>$ make clean && make deploy platform=android testflight=true notes="TestFlight for Android"</pre>

"notes" is optional.

Setting to use TestFlight, refer to "Build and deploy directly to TestFlight":#build-and-deploy-directly-to-testflight .
